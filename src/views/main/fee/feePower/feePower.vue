<template>
  <Table :TableList="tableList" :TableList2="List" :TableStyle="TableStyle">
    <template #payPowerStatus="scope">
      <el-tag type="success" v-if="scope.row.payPowerStatus == '1'"
        >已缴费</el-tag
      >
    ...
    <template #operation="scope">
      <div style="width: 100%; padding: 5px 0">
        ...
        <el-button type="warning" v-if="scope.row.payPowerStatus != '1'"
          >缴费</el-button
        >
      </div>
    </template>
...
const getList = async () => {
  try {
    const res = await getUserListApi("/feePower/list", "GET", {
      ...TableList,
    });
